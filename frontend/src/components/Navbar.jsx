...
const Navbar = () => {
  ...
  const menuItems = [
    { name: "Preschool", options: ["Pre School Tracing", "English", "Maths", "Science", "Homework", "Practice"] },
    { name: "Kindergarten", options: ["Nursery", "LKG", "UKG"] },
    { name: "1st Grade", options: ["1st Grade English", "1st Grade Maths", "1st Grade Science", "1st Grade Social Studies" ] },
    { name: "2nd Grade", options: ["2nd Grade English", "2nd Grade Maths", "2nd Grade Science", "2nd Grade Social Studies"] },
  ];
    ...
    <header className="relative z-[60]">
      ...
      <div className="fixed top-0 left-0 right-0 h-1 z-[60] pointer-events-none">
        <div
          ...
          style={{
            width: `${progress}%`,
            background: "linear-gradient(90deg, #ff5e5e, #ffcc33, #7a8873ff, #4dabff, #ff7ae0)",
...
>
<div className="relative max-w-[1350px] mx-auto flex items-center 
     justify-start lg:justify-between px-6">
...
<Link to="/">
  <motion.img
    ...
    className={`cursor-pointer transition-all ${scrolled ? "w-20" : "w-28"}`}
    ...
    >
      <div className="flex flex-col gap-3">
        {item.options.map((opt, i) => (
          <Link
  to={`/${item.name.toLowerCase().replace(/\s+/g, "-")}/${opt.toLowerCase().replace(/\s+/g, "-")}`}
>
  <motion.div
  ...
  >
    ...
    {opt}
          ...
          <div className="flex items-center gap-3 ml-auto lg:ml-0">
            ...
            {/* SEARCH — expands inline without overlapping */}
            <div className="relative flex items-center">
              <AnimatePresence>
                {openSearch && (
                  <motion.input
                    ...
                    placeholder="Search worksheets, grade or concept"
                    className="bg-white border border-blue-300 px-4 py-1 rounded-full shadow-md text-gray-700 outline-none mr-2"
                    autoFocus
                  />
                )}
              </AnimatePresence>
              ...
              <motion.button
                className="w-10 h-10 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 flex items-center justify-center shadow cursor-pointer hover:scale-110"
            ...
            <div className="hidden md:flex gap-2">
              <IconWrapper><FaInstagram className="text-pink-500" /></IconWrapper>
              <IconWrapper><FaFacebookF className="text-blue-600" /></IconWrapper>
              <IconWrapper><FaTwitter className="text-sky-500" /></IconWrapper>
            </div>
  ...
  >
    Privacy Policy
            ...
            {/* MOBILE MENU */}
            <button className="lg:hidden text-3xl ml-1" onClick={() => setMenuOpen((s) => !s)} aria-label="Open menu">
              {menuOpen ? <FiX /> : <FiMenu />}
            </button>
          </div>
        </div>
  ...
  {menuOpen && (
    ...
    >
        ...
        <details key={idx} className="border-b pb-2">
          ...
          <div className="pl-4 mt-2 flex flex-col gap-1">
            {item.options.map((opt, i) => (
              <Link
                key={i}
                to={`/${item.name.toLowerCase().replace(/\s+/g, "-")}/${opt
                  .toLowerCase()
                  .replace(/\s+/g, "-")}`}
      ...
      <Link to="/privacy-policy" onClick={() => setMenuOpen(false)}>
        <div className="mt-4 text-blue-600 font-semibold text-sm">Privacy Policy</div>
      </Link>
    </motion.div>
  )}
</AnimatePresence>
      ...
      {/* FLOATING DECORATIONS (balloons, crayons, stars) - positioned relative to header */}
      <div aria-hidden className="pointer-events-none">
...
const FloatingBalloon = ({ className = "", delay = 0 }) => (
  <motion.div
    ...
    style={{ zIndex: 30 }}
  >
    <svg width="80" height="110"  viewBox="0 0 64 96" fill="none" xmlns="http://www.w3.org/2000/svg">
      <ellipse cx="32" cy="34" rx="22" ry="28" fill="#ff7ae0" stroke="#e85ab2" strokeWidth="2" />
      <path d="M28 60 q4 8 8 0" stroke="#e85ab2" strokeWidth="2" fill="none" strokeLinecap="round" />
...
const FloatingCrayon = ({ className = "", delay = 0 }) => (
  ...
  >
    <svg width="72" height="32" viewBox="0 0 72 32" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="2" y="8" width="60" height="16" rx="4" fill="#ffd33d" stroke="#e6b800" strokeWidth="2" />
      <polygon points="62,8 70,16 62,24" fill="#ffd33d" stroke="#e6b800" strokeWidth="2" />
...
const FloatingStar = ({ className = "", delay = 0 }) => (
  ...
  >
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M12 2 L14.5 8.5 L21 9.2 L16 13.8 L17.2 20.5 L12 17.6 L6.8 20.5 L8 13.8 L3 9.2 L9.5 8.5 Z" fill="#ffcc33" stroke="#e6a600" strokeWidth="0.6"/>
    </svg>
  </motion.div>
);
